...
async def get_botohub_links(chat_id: int):
    """Получить список спонсорских ссылок от BotoHub"""
...
async def get_folders():
    async with aiosqlite.connect('bot.db') as db:
        async with db.execute("SELECT id, name FROM folders") as cursor:
...
async def get_contents_by_folder(folder_id: int):
    async with aiosqlite.connect('bot.db') as db:
        async with db.execute("SELECT id, title, url, photo_file_id, description FROM contents WHERE folder_id = ?", 
                              (folder_id,)) as cursor:
            return await cursor.fetchall()

async def get_content(content_id: int):
    async with aiosqlite.connect('bot.db') as db:
        async with db.execute('''SELECT id, folder_id, title, url, photo_file_id, photo_path, description 
                                 FROM contents WHERE id = ?''', (content_id,)) as cursor:
...
async def delete_folder(callback: CallbackQuery):
    ...
    async with aiosqlite.connect('bot.db') as db:
        async with db.execute("SELECT name FROM folders WHERE id = ?", (folder_id,)) as cur:
            folder_name = (await cur.fetchone())[0]
...
async def add_content_photo(message: Message, state: FSMContext, bot: Bot):
    ...
    if not file_ext:
        file_ext = '.jpg'
    local_filename = f"{photo.file_id}{file_ext}"
    local_path = f"{PHOTO_DIR}/{folder_name}/{local_filename}"
...
async def add_content_description(message: Message, state: FSMContext):
    ...
    await add_content(folder_id, title, url, photo_file_id, photo_path, description)
    await state.clear()
...
async def edit_content_menu(callback: CallbackQuery, state: FSMContext):
    ...
    kb = [
        [InlineKeyboardButton(text="✏️ Изменить название", callback_data="edit_ctitle")],
        [InlineKeyboardButton(text="🔗 Изменить ссылку", callback_data="edit_curl")],
        [InlineKeyboardButton(text="🖼 Изменить фото", callback_data="edit_cphoto")],
        [InlineKeyboardButton(text="📝 Изменить описание", callback_data="edit_cdesc")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_edit_content")]
    ]
    await callback.message.edit_text("Что вы хотите изменить?", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def edit_curl_prompt(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введите новую ссылку (любого формата):")
...
async def edit_curl_process(message: Message, state: FSMContext):
    ...
    url = message.text.strip()
    data = await state.get_data()
    await update_content_url(data['edit_content_id'], url)
    await state.clear()
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Получаем спонсорские ссылки от BotoHub
    ...
    if botohub_result and not botohub_result.get('skip', False):
        ...
        if tasks:
            # Формируем сообщение со спонсорскими ссылками
            sponsor_text = "🎁 <b>Рекомендуем подписаться на наших партнёров:</b>\n\n"
            
            # Создаём кнопки для каждой ссылки
            kb_buttons = []
            for i, task_url in enumerate(tasks, 1):
                kb_buttons.append([InlineKeyboardButton(text=f"🔗 Партнёр {i}", url=task_url)])
            ...
            await message.answer(sponsor_text, reply_markup=kb, parse_mode="HTML")
            return
    
    # Если нет спонсоров или API недоступен - показываем обычное меню
...
async def user_select_folder(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "📁 <b>Выберите контент для просмотра:</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=kb),
        parse_mode="HTML"
    )
...
async def user_select_content(callback: CallbackQuery, bot: Bot):
    ...
    url = content_data[3]
    photo_file_id = content_data[4]
    description = content_data[6]
...
async def main():
    await init_db()
    print("Бот запущен!")
    print(f"BotoHub интеграция: {'Активна' if BOTOHUB_TOKEN else 'Отключена'}")
